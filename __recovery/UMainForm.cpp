...
void AnalyzeAllModels(TStrings* FileList, const UnicodeString Archive) // Àíàëèçèðóåò âñå ôàéëû MDX è MDL (â íèõ ìîæíî íàéòè èìåíà ôàéëîâ òåêñòóð)
{
	...
	for(int i = 0; i < FileList->Count; i++)
	{
		...
		{
			WriteLog("Àíàëèç " + FileList->Strings[i] + "...");
			AnalyzeBinaryFile(s, Archive, FileList->Strings[i]);
			__s->Add(FileList->Strings[i]); // Ñïèñîê ïðîàíàëèçèðîâàííûõ ôàéëîâ
		}
	}
...
void AnalyzeAllTextFiles(TStrings* FileList, const UnicodeString Archive) // Àíàëèçèðóåò âñå ôàéëû TXT
{
	TStrings* s = new TStringList(), *__s = new TStringList();
	for(int i = 0; i < FileList->Count; i++)
	{
...
bool AnalyzeAllSLK(TStrings* List, const UnicodeString Archive)
{
	try
	{
		...
		LOOP(List->Count)
		{
			...
			WriteLog("Àíàëèç " + ExtractFileName(List->Strings[i]) + "...");
			...
			// Èùåì âñå ñòðîêîâûå êîíñòàíòû (õóéíþ â äâîéíûõ êàâû÷êàõ):
			...
			if(bDebugSaveNames) s->SaveToFile(GetTempDir() + "MPQRepacker\\" + ExtractFileName(List->Strings[i]) + ".txt"); // Îòëàäêà
			AutoSearch(s, Archive);
			MergeLists(s, List);
...
void AutoSearch(TStrings* List, const UnicodeString Archive)
{
	...
	for(int i = 0; i < List->Count; i++)
	{
		// Óãîëîê êîïèïàñòà
		Application->ProcessMessages();
		if(List->Strings[i].Pos("BTN")) // Èêîíêè êíîïîê
		{
...
bool InsertCheats(const UnicodeString War3mapJ, const UnicodeString CP, UnicodeString Activator)
{
	try
	{
		...
		while(s->Strings[nInsertIndex].SubString(1,5) == "local") nInsertIndex++; // Ïîñëå ëîêàëîâ
	...
	{
		Msg("Îøèáêà â ôóíêöèè InsertCheats: " + e.Message, true);
		return false;
    }
}
BOOL AddFileToMPQWithSFMPQ(const UnicodeString Archive, const UnicodeString File, const UnicodeString ArcFile, const unsigned nMaxFiles)
{
	try
	{
		...
		// Debug:
		if(bResult) WriteLog("Ôàéë " + ArcFile + " óïàêîâàí â àðõèâ ñ ïîìîùüþ SFMPQ!");
		else WriteLog("Îøèáêà óïàêîâêè ôàéëà ñ ïîìîùüþ SFMPQ.");
		//--------
		FreeDLL(hDll);
		return bResult;
	}
...
void TMainForm::FindCheatPacks()
{
	...
	if(cbbCheatPack->Items->Count < 1)
	{
		...
		cbbCheatPack->Text = "×èòû íå íàéäåíû";
...
bool ExtractUnknownFiles(UnicodeString Archive, TStrings* List, const UnicodeString Dir, bool bWriteLog)
{
	...
	if(nError == ERROR_SUCCESS)
	{
		...
		{
			if(FEntry[i].szFileName == NULL) // Èìÿ ôàéëà íå îïðåäåëåíî
			{
				HANDLE hFile = NULL;
				...
				if(FileNameIsValid(NewFilename) && !StrInList(List, NewFilename) && GetFileSize(hFile))
				{
					...
					{
						if(bWriteLog) WriteLog("Ðàñïàêîâêà " + NewFilename + "...");
...
void WriteWar3mapUnitsDOO(const UnicodeString Dir)
{
	...
	if(FileExists(FullPath)) return; // Íå çàìåíÿòü
	else
	{
		Classes::TFileStream* fs = new Classes::TFileStream(FullPath, fmCreate);
...
//		}
		//elseJFile->Insert(i, ""); // Âñòàâëÿåì ïóñòóþ ñòðîêó
...
bool CreateMapWithXDep(const UnicodeString CreatedMap, const UnicodeString DirWithFiles, bool bWriteLog)
{
	try
	{
		...
		ChDir(Dir);
		if(bWriteLog) WriteLog("Óïàêîâêà ôàéëîâ...");
...
//		CaptureConsoleOutput(Exe, "", MainForm->mmoLog);
		...
		{
			if(bWriteLog) WriteLog("Êàðòà óñïåøíî ñîáðàíà!");
		...
		{
			if(bWriteLog) WriteLog("Îøèáêà ñáîðêè êàðòû.");
			DeleteNonEmptyDir(Dir, MainForm->Handle);
			return false;
        }
	}
...
bool PackFilesToMPQ(UnicodeString Archive, const UnicodeString Dir = AppPath + "Temp\\")
{
	...
	WriteLog("Óïàêîâêà ôàéëîâ â " + ExtractFileName(Archive));
	Sleep(100);
	HANDLE hTempFile = 0;
	try
	{
	...
	for(int i = 0; i < s->Count; i++)
	{
		...
		if(AddFileToMPQ(hMpq, Dir + s->Strings[i], s->Strings[i]))
			WriteLog("Óïàêîâêà " + s->Strings[i] + "...");
		else
		{
			nError = GetLastError();
			WriteLog("Îøèáêà óïàêîâêè " + s->Strings[i] + " (Êîä îøèáêè: " + IntToStr(nError) + ")");
			if(hMpq != NULL)
			{
				SFileCloseArchive(hMpq);
            }
	...
	if(bWriteLog && !bResult)
	{
		...
		switch(nError)
		{
			...
			case ERROR_BAD_FORMAT:
				s = "Íåâîçìîæíî îòêðûòü ôàéë êàê MPQ àðõèâ!";
				break;
			case ERROR_NOT_ENOUGH_MEMORY:
				s = "Íå õâàòàåò ïàìÿòè!";
				break;
			case ERROR_AVI_FILE:
				s = "AVI ôàéë ñ ðàñøèðåíèåì MPQ!";
				break;
			case ERROR_ACCESS_DENIED:
				s = "Îòêàçàíî â äîñòóïå.";
				break;
			case ERROR_SHARING_VIOLATION:
			case ERROR_LOCK_VIOLATION:
				s = "Ôàéë óæå îòêðûò â äðóãîé ïðîãðàììå.";
				break;
			default:
				s = "Íåèçâåñòíàÿ îøèáêà! (Êîä îøèáêè: " + IntToStr(nError) + ")";
		}
		s = "Îøèáêà: " + s;
		WriteLog(s);
	}
...
bool ExtractAllFiles(const UnicodeString Archive, bool bRewriteW3I, bool bRewriteDOO, bool bUnpackUnknown, const UnicodeString Dir, const UnicodeString Listfile, bool bWriteLog)
{
	try
	{
		...
		{
			if(bWriteLog) WriteLog("Ôàéë " + ExtractFileName(Archive) + " íå íàéäåí!");
			return false;
		}
		...
		if(!PluginManager->Overrides.Search.Enabled)
		{
			...
			if(FileExists(Listfile))
			{
				...
				if(bWriteLog) WriteLog("Çàãðóæåí âíåøíèé ëèñòôàéë: " + Listfile);
			}
			...
			if(bAnalyze[ANALYZE_JSCRIPT])
			{
				...
				if(bWriteLog) WriteLog("Àíàëèç war3map.j...");
				AutoSearch(s, Archive);
			}
			if(bAnalyze[ANALYZE_W3_BIN])
			{
				...
				{
					Application->ProcessMessages();
					if(VerifyFile(Archive, BinaryFiles[i]) && AnalyzeBinaryFile(s, Archive, BinaryFiles[i], false) && bWriteLog) WriteLog("Àíàëèç " + BinaryFiles[i] + "...");
				}
			}
            if(bAnalyze[ANALYZE_IMPLIST])
			{
				AnalyzeBinaryFile(s, Archive, "war3map.imp");
				if(bWriteLog) WriteLog("Àíàëèç war3map.imp...");
			}
			AutoSearch(s, Archive);
			...
			if(bWriteLog) WriteLog("Ãåíåðàöèÿ war3map.imp...");
			GenerateWar3mapIMP(s, Archive, Dir);
			// Ðàñïàêîâêà íåèçâåñòíûõ ôàéëîâ:
			if(bUnpackUnknown) ExtractUnknownFiles(Archive, s, Dir);
			// Ðàñïàêîâêà èçâåñòíûõ ôàéëîâ:
		...
		if(!PluginManager->Overrides.Unpack.Enabled)
		{
			...
			{
				WriteLog("Îøèáêà ðàñïàêîâêè ôàéëîâ.");
				return 0;
            }
			for(int i = 0; i < s->Count; i++)
			{
				...
				if(!StrInList(s1, s->Strings[i]))
				{
					...
					if(r == ERROR_SUCCESS)
					{
						if(bWriteLog) WriteLog("Ðàñïàêîâêà " + s->Strings[i] + "...");
						s1->Add(s->Strings[i]);
						nLastUnpackedFilesCount++;
					}
				}
			...
			if(bWriteLog) WriteLog("Ðàñïàêîâàíî " + IntToStr((int)nLastUnpackedFilesCount) + " ôàéëîâ. (" + FilesFound + ")");
		}
		else
		{
...
bool AnalyzeMainScript(TStrings* List, const UnicodeString Archive)
{
	try
	{
		...
		if(bDebugSaveNames) s->SaveToFile(GetTempDir() + "MPQRepacker\\war3map.j.txt"); // Îòëàäêà
		AutoSearch(s, Archive);
		MergeLists(s, List);
		s->Free();
		DeleteFile(ScriptFile);
...
bool AnalyzeTextFile(TStrings* List, const UnicodeString Archive, const UnicodeString File)
{
	try
	{
		...
		// Èùåì âñå ñòðîêè ìåæäó ðàâíî è êîíöîì ñòðîêè
		...
		AutoSearch(s, Archive);
		MergeLists(s, List);
		if(bDebugSaveNames)	s->SaveToFile(GetTempDir() + "MPQRepacker\\" + ExtractFileName(File) + ".txt"); // Îòëàäêà
		s->Free();
		DeleteFile(ScriptFile);
		return true;
	}
...
bool AnalyzeBinaryFile(TStrings* List, const UnicodeString Archive, const UnicodeString File, bool bLocalFile)
{
	try
	{
		...
		if(!bLocalFile)
		{
			...
			{
                WriteLog("Îøèáêà ðàñïàêîâêè ôàéëà äëÿ àíàëèçà: " + File);
				VCL_FREE(s);
				return false;
			}
		}
		...
		if(dwHeader == 0x5733504D || dwHeader == 0x31504C42) return false; // MP3/BLP ôàéë
		...
		nBufferSize = fs->Size;
		VCL_FREE(fs);
		// Èùåì âñå ñòðîêè ìåæäó NULL'àìè:
...
{
	DWORD nPos = 8 + nDefaultMapNameSize; // nDefaultMapNameSize - äëèííà ñòàíäàðòíîãî íàçâàíèÿ êàðòû
...
UnicodeString FindTriggerString(const UnicodeString War3mapWTS, const UnicodeString Num)
{
	...
	int i = FindStr(s, "STRING " + Num) + 2; // Ïîñëå ýòîé ñòðîêè è {
	UnicodeString Result = "";
	while(s->Strings[i] != "}")
	{
	...
	const bool bDeleteTempDir, const UnicodeString ListFile, UnicodeString TempDir)
{
	...
	if(!bResult)
	{
		// WriteLog("Îøèáêà ðàñïàêîâêè: ôàéë " + ExtractFileName(SourceMap) + " íå ÿâëÿåòñÿ MPQ àðõèâîì.");
		WriteLog("Îøèáêà ðàñïàêîâêè: íåâîçìîæíî îòêðûòü ôàéë " + ExtractFileName(SourceMap) + "!");
		return false;
	}
	...
	WriteLog("\r\nÏåðåïàêîâêà êàðòû " + ExtractFileName(SourceMap) + ":");
	bResult = ExtractAllFiles(SourceMap, bPatchW3I, bEmptyDOO, bUnpackUnknown, TempDir, ListFile);
	if(!bResult)
	{
		WriteLog("Îøèáêà ðàñïàêîâêè. Îïåðàöèÿ ïðåðâàíà.");
		return false;
	}
	DeleteFile(DestMap);
...
//			MapName = ReservedSize(DeleteTransfers(MapName), nDefaultMapNameSize);
			if(MapName.Length()) WriteLog("Ðåàëüíîå èìÿ êàðòû íàéäåíî: " + MapName);
			else
			{
				Msg("Ðåàëüíîå èìÿ êàðòû íå íàéäåíî!", true);
				InputBox("Èìÿ êàðòû", "Ââåäèòå ðåàëüíîå èìÿ êàðòû:", "");
			}
		}
	...
	if(bInsertCheats)
	{
		WriteLog("Âñòàâêà ÷èò-ïàêà...");
		if(InsertCheats(TempDir + "war3map.j", CheatPack, Activator))
			WriteLog("×èò-ïàê óñïåøíî âñòàâëåí â ñêðèïò êàðòû!");
		else
			WriteLog("Îøèáêà âñòàâêè ÷èò-ïàêà.");
    }
	if(bManualEdit) Msg("Âíåñèòå íåîáõîäèìûå ïîïðàâêè â ôàéëû êàðòû è íàæìèòå ÎÊ.");
	...
	if(bMapRename)
	{
		...
		else WriteLog("Îøèáêà: Óïàêîâàííàÿ êàðòà íå íàéäåíà!");
	}
	...
	if(!bResult) WriteLog("Îøèáêà óïàêîâêè!");
	else WriteLog("Ïåðåïàêîâêà êàðòû çàâåðøåíà!");
...
void __fastcall TMainForm::btnRePackClick(TObject *Sender)
{
	...
	try
	{
		...
		{
			Msg("Ôàéë íå íàéäåí: " + edtSourceMap->Text, true);
			// return;
		}
		else
		{
...
void TMainForm::LoadSettings()
{
	...
	if(R->OpenKey("SOFTWARE\\Blizzard Entertainment\\Warcraft III", false))
	{
		...
		if(FileExists(War3Path + "war3.exe")) // Ïðîâåðêà ïðàâèëüíîñòè ïóòè
		{
...
void __fastcall TMainForm::btnCommandsClick(TObject *Sender)
{
	...
	{
		Msg("Ôàéë commands.txt íå íàéäåí", true);
    }
}
//---------------------------------------------------------------------------
...
void __fastcall TMainForm::btnBatchRepackClick(TObject *Sender)
{
	...
	bInput = InputQuery("Ïàêåòíàÿ ïåðåïàêîâêà", "Ââåäèòå ïàïêó ñ èñõîäíûìè ôàéëàìè:", tmp);
	if(!bInput) return;

	const UnicodeString SourceDir = IncludeTrailingBackslash(tmp);
	if(!DirectoryExists(SourceDir))
	{
    	WriteLog("Äèðåêòîðèÿ íå ñóùåñòâóåò: " + SourceDir);
		return;
	}
	tmp = ExtractFilePath(edtDestMap->Text);
	bInput = InputQuery("Ïàêåòíàÿ ïåðåïàêîâêà", "Ââåäèòå ïàïêó äëÿ ïåðåïàêîâàííûõ ôàéëîâ:", tmp);
	if(!bInput) return;
	const UnicodeString DestDir = IncludeTrailingBackslash(tmp);
	if(!__MkDir(DestDir))
	{
		WriteLog("Îøèáêà ñîçäàíèÿ äèðåêòîðèè: " + DestDir);
		return;
    }
	const bool bSubDirs = Ask("Ïåðåïàêîâûâàòü êàðòû â ïîäïàïêàõ?");
	TStrings* s = new TStringList();
	...
	this->Enabled = true;
	__Status = mrsIdle;
	Msg("Ïàêåòíàÿ ïåðåïàêîâêà çàâåðøåíà!");
}
//---------------------------------------------------------------------------
...
void __fastcall TMainForm::btnTestMapClick(TObject *Sender)
{
	...
	if(!FileExists(War3Exe)) Msg("Ôàéë war3.exe íå íàéäåí!", true);
	else ExecuteProcess(War3Exe, "", 0);
	/*else if(!FileExists(edtDestMap->Text)) Msg("Ôàéë êàðòû íå íàéäåí!", true);
	else
	{
